// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"strconv"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// UserUserGroupListResponse user user group list response
//
// swagger:model userUserGroupListResponse
type UserUserGroupListResponse struct {

	// usergroup
	Usergroup []*UserUserGroupPrivateResponse `json:"usergroup"`
}

// Validate validates this user user group list response
func (m *UserUserGroupListResponse) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateUsergroup(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *UserUserGroupListResponse) validateUsergroup(formats strfmt.Registry) error {
	if swag.IsZero(m.Usergroup) { // not required
		return nil
	}

	for i := 0; i < len(m.Usergroup); i++ {
		if swag.IsZero(m.Usergroup[i]) { // not required
			continue
		}

		if m.Usergroup[i] != nil {
			if err := m.Usergroup[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("usergroup" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// ContextValidate validate this user user group list response based on the context it is used
func (m *UserUserGroupListResponse) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidateUsergroup(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *UserUserGroupListResponse) contextValidateUsergroup(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.Usergroup); i++ {

		if m.Usergroup[i] != nil {
			if err := m.Usergroup[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("usergroup" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (m *UserUserGroupListResponse) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *UserUserGroupListResponse) UnmarshalBinary(b []byte) error {
	var res UserUserGroupListResponse
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
